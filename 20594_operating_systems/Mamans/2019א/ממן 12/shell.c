/*Intro
Description:    This library contains implementation of job scheduler.
...
#define MAX_COMMAND_LEN 250     // max length of a single command string
...
void globLastArgument(struct childProgram * prog, int * argcPtr, int * argcAllocedPtr) {
    int argc, argcAlloced, rc, flags, i;
    char * src, * dst;

    argc = *argcPtr;
    argcAlloced = *argcAllocedPtr;
    ...
    if (rc == GLOB_NOSPACE) {
    ...
    else if (rc == GLOB_NOMATCH || (!rc && (prog->globResult.gl_pathc - i) == 1 && !strcmp(prog->argv[argc - 1], prog->globResult.gl_pathv[i]))) {
        ...
        *dst = '\0';
        ...
        memcpy(prog->argv + (argc - 1), prog->globResult.gl_pathv + i, sizeof(*(prog->argv)) * (prog->globResult.gl_pathc - i));
...
int parseCommand(char ** commandPtr, struct job * job, int * isBg) {
    ...
    char *command, *src, *buf;
    char quote = '\0';
    int argc, done, argvAlloced, count;
    argc = done = 0;
    ...
    *isBg = 0;
    ...
    /* We set the argv elements to point inside of this string. The memory is freed by freeJob().
       Getting clean memory relieves us of the task of NULL terminating things and makes the rest of this look a bit cleaner
       (though it is, admittedly, a tad less efficient) */
    ...
    while (*src && !done) {
        if (quote == *src)
          ...
          case '|':                         /* pipe */
            ...
            /* and start the next */
            job->numProgs++; 
            job->progs = realloc(job->progs, sizeof(*job->progs) * job->numProgs);
...
int runCommand(struct job newJob, struct jobSet *jobList, int inBg) {
    struct job *job;
    char *newdir, *buf, *statusString;
    int i, len, nextin, nextout, pipefds[2];

    // handle built-ins here -- we don't fork() so we can't background these very easily
    if (!strcmp(newJob.progs[0].argv[0], "exit"))
        exit(0); // this should return a real exit code
    ...
    else if (!strcmp(newJob.progs[0].argv[0], "fg") || !strcmp(newJob.progs[0].argv[0], "bg"))
    {
        ...
        //Get the job first argument
        parm2=newJob.progs[0].argv[1];

        //Check number of parameters<=2
        if(parm2==NULL || newJob.progs[0].argv[2]!=NULL)
        {
            fprintf(stderr, "%s: Exactly one argument is required.\n", newJob.progs[0].argv[0]);
            return 1;
        }

        //Check the syntax of the first argument
        for(i=0; parm2[i];i++) {
            if((i==0 && parm2[i]!='%') || (i!=0 && !isdigit(parm2[i]))) {
                fprintf(stderr, "%s: '%s' bad argument.\n", newJob.progs[0].argv[0], newJob.progs[0].argv[1]);
                return 1;
            }
        }

        //Get the job number out of the first argument
        if(!(jobNum = atoi(parm2+1))) {
            fprintf(stderr, "%s: Failed to interpert job number on argument %s.\n", newJob.progs[0].argv[0], newJob.progs[0].argv[1]);
            return 1;
        }

        //Searches for the job
        for(job=jobList->head; job && job->jobId!=jobNum; job=job->next);
        if(!job) {
            //Didnt found the job
            fprintf(stderr, "%s: Didnt found job number %d.\n", newJob.progs[0].argv[0], jobNum);
            return 1;
        }

        //fg is the command
        if (!strcmp(newJob.progs[0].argv[0], "fg")) {
            //Set this job to be the running job on the jobList
            jobList->fg=job;

            //Set it to run in the foreground
            if (tcsetpgrp(0, job->pgrp))
            {
                fprintf(stderr, "Failed in tcsetpgrp function.\n");
                perror("tcsetpgrp");
                return 1;
            }
        }

        //In any case, Continue the job
        if(kill(job->pgrp, SIGCONT)) {
            fprintf(stderr, "Failed to continue job number %d.\n", jobNum);
            return 1;
        }

        //In any case, zero its stopped programs on the jobList
    ...
    for (i = 0; i < newJob.numProgs; i++)
    {
        ...
        /* put our child in the process group whose leader is the first process in this pipe */
        setpgid(newJob.progs[i].pid, newJob.progs[0].pid);
...
int main(int argc, char ** argv)
{
    ...
    if (argc > 2)
    {
        fprintf(stderr, "unexpected arguments; usage: <commands>\n");
        exit(1);
    }
    else if (argc == 2)
    {
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    printf("\n" JOB_STATUS_FORMAT, jobList.fg->jobId, "Stopped", jobList.fg->text);
                    jobList.fg = NULL;
                }
            }
