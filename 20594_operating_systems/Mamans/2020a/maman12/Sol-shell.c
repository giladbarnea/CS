...
int runCommand(struct job newJob, struct jobSet * jobList, 
	...
	for (job = jobList->head; job; job = job->next) {
		...
		// Check if program running or stopped
		if (job->numProgs == job->stoppedProgs) 
		{
			statusString = "Stopped";		
		}
		else if (job->numProgs == job->runningProgs) 
		{
	...
	if (newJob.progs[0].argv[1] == 0 ||
		...
		(newJob.progs[0].argv[1])[0] != '%')
	{
		return 1;	
	}
	
  	// else find the job in the job list 
	int jobId = atoi(newJob.progs[0].argv[1] + 1);

	// Find the job in the job list
	job = findJobById(jobId, jobList);

	// Check if job found, if not-return 1
	if (!job)
	{
		return 1;	
	}
